...
class missaoC(ImageModule,Aruco3d,MobileNetDetector):
    def __init__(self,args):
        ...
        self.entrouparaprimeiracaixa = 0
        ...
        self.target_time_andada = 10000

        self.centro_caixa = 0

        self.caixavermelha = 0

        self.virardireitacaixa1= 0

        self.virarparadireita=0 
    ...
    # Subscribers
        ...
        self.robot_machine = {
            ...
            "slalom": self.slalom,
            "finalslalom": self.finalslalom,
            "vaparaid100": self.vaparaid100  
    ...
    def color_segmentation(self, hsv: np.ndarray, lower_hsv: np.ndarray, upper_hsv: np.ndarray,id,) -> Point:
        ...
        if self.caixaazul==0 and self.caixavermelha == 0:
        ...
        elif self.caixaazul==1 or self.caixavermelha==1:
            cv2.imshow("ollla",mask)
            cv2.waitKey(1)
    ...
    def image_callback(self, msg: CompressedImage) -> None:
        ...
        if len(self.resultsAruco) > 0:
            if self.id250==1:
                for arucos in self.resultsAruco:
                    if arucos["id"][0]==250:
                        self.centro_aruco = arucos["centro"]
                        self.point.x = self.centro_aruco[0]            
            print(self.id)            
            
            self.id = self.resultsAruco[0]['id'][0]
            self.centro_aruco = self.resultsAruco[0]['centro']
            #print(self.centro_aruco)
        else:
            self.id = -100    
        ...
        if self.args.cor == "azul" and self.caixaazul==0 and self.id250!=1:
        ...
        elif self.args.cor == "verde" and self.caixaazul==0 and self.id250!=1:
        ...
        elif self.args.cor == "vermelho" and self.caixaazul==0 and self.id250!=1:
        ...
        if self.centro_a_ser_guiado!=-1 and self.caixaazul==0 and self.id250!=1:
            self.point.x = self.centro_a_ser_guiado[0] -35
        if self.caixaazul==1 and self.id250!=1:
            ...
            if self.centro_caixa!=-1 and self.id250!=1:
                self.point.x = self.centro_caixa[0]
        if self.caixavermelha ==1 and self.id250!=1:
            self.centro_caixa_vermelha = self.color_segmentation(cv_image,self.color_param["CaixaVermelha"]["lower"],self.color_param['CaixaVermelha']['upper'], self.args.id)
            if self.centro_caixa_vermelha!=-1 and self.id250!=1:
                self.point.x = self.centro_caixa_vermelha[0]       
    ...
    def procura(self):
        ...
        if self.timecurvainicial ==0:
            self.get_error()

       

        if np.min(self.frente)<0.55  or self.timecurvainicial==1:
            print("chegueii")
            if self.timecurvainicial == 0:
                self.timecurvainicial = 1
                self.twist.linear.x = 0
                self.twist.angular.z = 0.5
                self.target_time21 = rospy.Time.now().to_sec() + 2.5
            if rospy.Time.now().to_sec()>= self.target_time21:     
                self.id250 = 0
                self.caixaazul=1
                self.twist.linear.x = 0
                ...
                while True:
                    if self.achacaixa == 0:
                        self.target_timeCaixaAzul = rospy.Time.now().to_sec() + 55
                        self.achacaixa = 1
                    if rospy.Time.now().to_sec()>= self.target_timeCaixaAzul:
                        break
    ...
    def slalom(self):
        ...
        if rospy.Time.now().to_sec()>= self.target_time_andada or self.entrouparaprimeiracaixa == 0:
            if self.virardireitacaixa1==0 or self.caixavermelha==1:
                print("corrigindo")
                self.get_error()
        if self.primeiraentrada ==0:
            self.twist.linear.x = 0.09
            self.caixaazul = 1
            self.primeiraentrada = 1
        if (np.min(self.frente)<0.45 and self.caixavermelha == 0 and self.entrouparaprimeiracaixa == 0) or self.virardireitacaixa1==1:
            
            self.twist.linear.x = 0
            
            
            if self.virardireitacaixa1==0:
                self.twist.angular.z =0.5
                self.virardireitacaixa1 = 1
                self.target_time19 = rospy.Time.now().to_sec() + 1.5
            if rospy.Time.now().to_sec()>=self.target_time19:
                self.twist.angular.z = 0
                self.twist.linear.x = 0.09
                self.target_time_andada = rospy.Time.now().to_sec() + 2.5
                self.entrouparaprimeiracaixa = 1    
                self.caixavermelha = 1
                self.caixaazul = 0
                self.virardireitacaixa1=0

        if np.min(self.frente)<0.40 and self.caixavermelha==1 and rospy.Time.now().to_sec()>= self.target_time_andada:
            self.twist.linear.x = 0
            self.twist.angular.z = 0
            self.robot_state = "finalslalom"


    def finalslalom (self):
        if self.virarparadireita==0:
            self.twist.angular.z =-0.5
            self.virarparadireita = 1
            self.target_time20 = rospy.Time.now().to_sec() + 1.5
        if rospy.Time.now().to_sec()>= self.target_time20:
            if self.andarfinal==0:
                self.twist.angular.z =0
                self.twist.linear.x = 0.09
                self.andarfinal = 1
                self.target_time_andadafinal = rospy.Time.now().to_sec() + 1.8
            if rospy.Time.now().to_sec()>= self.target_time_andadafinal:
                while True:
                    self.pista.robot_state = "anda"
                    self.pista.control()
                    if self.achar100==0:
                        self.target_time_id100 = rospy.Time.now().to_sec() + 10
                        self.achar100 = 1

                    if rospy.Time.now().to_sec() >= self.target_time_id100:
                        break
                self.pista.robot_state = "parar"
                self.twist.linear.x = 0
                self.twist.angular.z = 0     
                self.robot_state = "vaparaid100"
    def vaparaid100(self):
        if self.virardireitaparaid==0:
            self.virardireitaparaid = 1
            self.target_time22 = rospy.Time.now().to_sec() + 1.8
            self.twist.angular.z = -0.5
        if rospy.Time.now().to_sec()>= self.target_time22:
            self.pista.robot_state = "anda"
            while True:
                self.pista.control()
                if self.id==100:
                    self.id100 = 1
                if self.id100==1 and self.id<0:
                    break    
            self.pista.robot_state = "parar"
            self.twist.linear.x = 0
            self.twist.angular.z = 0
    ...
    def virar(self):
        ...
        if self.entrouu == 1:
            ...
            self.target_time = rospy.Time.now().to_sec() + 10.0
            self.twist.linear.x = 0.09
            self.entrouu = 0
